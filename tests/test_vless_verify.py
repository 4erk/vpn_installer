...
class VlessVerifyTests(unittest.TestCase):
    ...
    def test_runner_separates_uncapped_capacity_from_bounded_stability(self) -> None:
        ...
        self.assertIn("attempt_seconds=10", runner)
        ...
        self.assertIn("phase=capacity", runner)
        self.assertIn("phase=stability", runner)
        self.assertIn('source_index=$((${#throughput_urls[@]} - 1))', runner)
        self.assertIn('if [[ "$phase" == "capacity" ]]', runner)
        self.assertIn("throughput-capacity-boundary", runner)
        self.assertIn("--limit-rate 2000000", runner)
        self.assertIn('"stability_bytes_per_second"', runner)
