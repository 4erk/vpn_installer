...
class AdminWebTests(unittest.TestCase):
    ...
    def test_apply_rules_inserts_adaptive_literal_override_before_catchall(self) -> None:
        env = generate_default_env("demo")
        env["RU_PUBLIC_IP"] = "203.0.113.10"
        env["FOREIGN_PUBLIC_IP"] = "198.51.100.20"
        base = json.loads(render_ru_singbox(env))
        adaptive_rule = admin_apply.normalize_adaptive_rule(
            {
                "id": "slow-live",
                "value": "91.108.56.103/32",
                "outbound": "to-foreign",
                "expires_at": 4_000_000_000,
            },
            now=1_700_000_000,
        )
        config = admin_apply.apply_admin_rules_to_config(base, [], [adaptive_rule])
        route_rules = config["route"]["rules"]
        adaptive_index = next(i for i, rule in enumerate(route_rules) if rule.get("ip_cidr") == ["91.108.56.103/32"])
        catchall_index = next(i for i, rule in enumerate(route_rules) if rule.get("ip_cidr") == ["0.0.0.0/0"])

        self.assertLess(adaptive_index, catchall_index)
        self.assertEqual(route_rules[adaptive_index]["outbound"], "to-foreign")

    def test_load_adaptive_rules_drops_expired_and_bad_rules(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rules_path.write_text(
                json.dumps(
                    {
                        "rules": [
                            {"id": "ok", "value": "91.108.56.103/32", "outbound": "to-foreign", "expires_at": 4_000_000_000},
                            {"id": "expired", "value": "91.108.56.104/32", "outbound": "to-foreign", "expires_at": 1},
                            {"id": "bad", "value": "not-cidr", "outbound": "to-foreign"},
                            {"id": "bad-outbound", "value": "91.108.56.105/32", "outbound": "direct-ru"},
                        ]
                    }
                ),
                encoding="utf-8",
            )
            rules = admin_apply.load_adaptive_rules(rules_path, now=1_700_000_000)

        self.assertEqual([rule["id"] for rule in rules], ["ok"])

    def test_upsert_adaptive_rule_preserves_other_fresh_rules(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            rules_path = Path(tmp) / "adaptive.json"
            admin_apply.upsert_adaptive_rule(
                "91.108.56.103/32",
                "to-foreign",
                "first",
                3600,
            ...
            rules = admin_apply.upsert_adaptive_rule(
                "149.154.167.51/32",
                "to-foreign",
                "second",
                3600,
    ...
    def test_admin_apply_cli_can_add_adaptive_rule_without_restart(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("vpn_installer.admin_apply.shutil.which", return_value=None), patch("vpn_installer.admin_apply.subprocess.run") as run_mock:
                self.assertEqual(
                    admin_apply.main(
                        [
                            "--base",
            ...
            adaptive_rule = next(rule for rule in payload["route"]["rules"] if rule.get("ip_cidr") == ["91.108.56.103/32"])

        self.assertEqual(adaptive_rule["outbound"], "to-foreign")
        run_mock.assert_not_called()
