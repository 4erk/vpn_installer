...
class ClientDriftTests(unittest.TestCase):
    ...
    def test_detects_stale_hiddify_json_port(self) -> None:
        env = self.make_env()
        payload = {"outbounds": [{"type": "vless", "server": env["RU_PUBLIC_IP"], "server_port": 443, "uuid": env["CLIENT_UUID"]}]}
    ...
    def test_detects_stale_vless_uri_port(self) -> None:
        env = self.make_env()
        uri = f"vless://{env['CLIENT_UUID']}@{env['RU_PUBLIC_IP']}:443?security=reality#demo\n"
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_current_profile_has_no_findings(self) -> None:
        env = self.make_env()
        payload = {"outbounds": [{"type": "vless", "server": env["RU_PUBLIC_IP"], "server_port": 8443, "uuid": env["CLIENT_UUID"], "public_key": env["RU_REALITY_PUBLIC_KEY"], "short_id": env["RU_REALITY_SHORT_ID"]}]}
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_invalid_json_falls_back_to_uri_scan(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text("{" + f"vless://{env['CLIENT_UUID']}@{env['RU_PUBLIC_IP']}:443", encoding="utf-8")
            findings = find_client_drift(env, [path])
        self.assertEqual(len(findings), 1)
