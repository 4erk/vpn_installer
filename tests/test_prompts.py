...
class PromptTests(unittest.TestCase):
    ...
    def test_runtime_password_uses_canonical_node_env_names(self) -> None:
        gateway = RemoteTarget(node_id=NODE_GATEWAY, auth_mode="password")
        exit_target = RemoteTarget(node_id=NODE_EXIT, auth_mode="password")
        with patch.dict(
            "os.environ",
            {
    ...
    def test_ask_install_action_branches(self) -> None:
        with patch("vpn_installer.prompts.prompt_choice", return_value="install") as mocked:
            self.assertEqual(ask_install_action(NODE_GATEWAY, "demo", {"installed": "0"}), "install")
        mocked.assert_called_once()

        with patch("vpn_installer.prompts.prompt_choice", return_value="skip") as mocked:
            self.assertEqual(ask_install_action(NODE_GATEWAY, "demo", {"installed": "1", "node": NODE_EXIT, "deployment_name": "other"}), "skip")
        mocked.assert_called_once()
