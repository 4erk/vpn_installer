...
class AdaptiveTests(unittest.TestCase):
    ...
    def test_shell_fragments_are_generated_from_route_fail_classes(self) -> None:
        ...
        self.assertIn("maybe_adapt_ipv4_literal_route()", collector)
        self.assertIn("maybe_drop_adaptive_ipv4_route()", collector)
        self.assertIn("--add-adaptive-cidr", collector)
        self.assertIn("--remove-adaptive-cidr", collector)
        self.assertIn('maybe_adapt_ipv4_literal_route "${ipv4_literal_top}"', collector)
        self.assertIn('maybe_drop_adaptive_ipv4_route "${domain_foreign_top}"', collector)
