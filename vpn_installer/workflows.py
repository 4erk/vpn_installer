...
def finalize_install_output(env: dict[str, str], deployment_name: str) -> None:
    ...
    print(f"Адаптивный TCP/QUIC профиль для Hiddify: {paths['hiddify_json']}")
    print(f"Адаптивный Android профиль для Hiddify: {paths['android_hiddify_json']}")
    ...
    print(f"4. Для автоматического обхода деградации TCP в Hiddify используй {paths['hiddify_json'].name}; URI {paths['hiddify_uri_compat'].name} остаётся основным совместимым контрактом.")
