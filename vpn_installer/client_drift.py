...
def _inspect_json(
    ...
    for item in _walk_dicts(payload):
        ...
        is_current_profile = server == gateway_public_ip or uuid_value == expected_uuid or public_key == expected_public_key
        if not is_current_profile:
            continue
        port = _as_int(item.get("server_port", item.get("port")))
        if server == gateway_public_ip and port is not None and port != expected_port:
...
def _inspect_text(
    ...
    escaped_gateway_ip = re.escape(gateway_public_ip)
    expected_port = env.get("RU_LISTEN_PORT", "").strip()
    if not expected_port:
        return []
    findings: list[ClientDriftFinding] = []
    for match in re.finditer(rf"vless://[^@\s]+@{escaped_gateway_ip}:(\d+)", text):
        port = match.group(1)
...
def find_client_drift(env: dict[str, str], paths: Iterable[Path] | None = None) -> list[ClientDriftFinding]:
    gateway_public_ip = _gateway_public_ip(env)
    findings: list[ClientDriftFinding] = []
    for path in paths if paths is not None else default_candidate_paths():
        ...
        if stripped.startswith("{") or stripped.startswith("["):
            try:
                findings.extend(_inspect_json(path, json.loads(text), env, gateway_public_ip))
            except json.JSONDecodeError:
                findings.extend(_inspect_text(path, text, env, gateway_public_ip))
        else:
            findings.extend(_inspect_text(path, text, env, gateway_public_ip))
