...
from vpn_installer.common import cli_entrypoint, error_summary  # noqa: E402
...
def run(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"\nВвод прерван. Запусти {cli_entrypoint()} без аргументов или передай команду явно.", file=sys.stderr)
        if log_path:
            print(f"Подробности сохранены в: {log_path}", file=sys.stderr)
        return 1
    except Exception as exc:  # noqa: BLE001
        if _is_audit_failure(exc):
            print(f"Самопроверка завершилась с ошибкой: {error_summary(exc)}", file=sys.stderr)
        ...
        print(f"Непредвиденная ошибка: {error_summary(exc)}", file=sys.stderr)
        if log_path:
            print(f"Подробности сохранены в: {log_path}", file=sys.stderr)
        return 1
