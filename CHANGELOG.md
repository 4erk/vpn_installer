...
- Свежий JSON больше не обновляет возраст старых измерений: agent сохраняет время получения каждого collector, фиксирует границы журналов, а verifier отклоняет отсутствующие, устаревшие и будущие доказательства.
- Public VLESS acceptance связывает свежие Xray events с TCP-сокетами своего эфемерного процесса. Чужое соединение того же IP/NAT и историческая запись больше не подтверждают проверку; неполное наблюдение явно остаётся inconclusive.
- Классификатор различает запрошенное назначение, адрес неудачного соединения и фазу ошибки. Разрешённый в IP домен не теряет доменную категорию при наличии request trace; без контекста сохраняется неизвестная категория с адресом и исходной строкой. Контекст применяется до разделения на временные окна. Ошибка чтения существующего VLESS-соединения не считается неудачей подключения.

### Changed

- `diagnose telegram --destination <IP>` проверяет анонимный ответ MTProto через router и непосредственно с exit. SOCKS accept не считается ответом Telegram; отчёт сохраняет отдельные фазы и отрицательные результаты. Вход в аккаунт, фоновый сервис, новые маршруты и подмена Telegram DC не используются.
- Unit и coverage используют один discovery runner. `unit-results.json` содержит выполненные test IDs, ошибки и причины пропуска; пустой запуск считается ошибкой.

### Compatibility

- Обновление только с `0.22.5`, fresh install и reinstall `0.22.6`. Schemas `3/5/6`, основной VLESS URI, web-admin, routing policy и межсерверная адаптация не меняются.
- Проверка MTProto подтверждает доступность протокола, но не аутентификацию сервера, загрузку медиа или путь конкретного телефона. Недоступность назначения уже с exit не устраняется зелёной HTTP-проверкой или переключением между транспортами к тому же exit.

## [0.22.5] - 2026-09-05

### Fixed

- Штатная SSH-установка компилирует каждый выбранный узел по его preflight platform facts. Ожидаемый manifest входит в загружаемый архив; сервер сверяет с ним свой render до установки пакетов и переключения runtime. Проверка результата использует ID из этого архива, а не отдельный preview. Это устраняет ложное несовпадение релиза при отличии ОС от платформы локальной сборки.
- SSH exec и stream используют общий цикл чтения и единый deadline для connect/retry, channel request, stdin и ожидания завершения. SFTP setup и полная загрузка также ограничены общим бюджетом: понемногу поступающие данные больше не продлевают операцию бесконечно. При истечении закрывается управляющий transport; автономная server transaction продолжает работать по прежнему контракту.
- SELinux file contexts и оба протокола DNS-порта проверяются до изменения policy. Чужой type не перезаписывается, существующие совпадающие правила не изменяются. Ошибка подготовки отменяет только её успешные добавления; ошибка обратной операции явно сообщается. Полный откат host policy после успешной подготовки остаётся отдельным непроверенным ограничением, описанным в `docs/PLATFORMS.md`.

### Changed

- GitHub release actions обновлены до Node 24 и закреплены точными commit SHA. Публикация по-прежнему требует полного audit; версия action не выбирается плавающим тегом.

### Compatibility

- Обновление только с `0.22.4`, fresh install и reinstall `0.22.5`. Schemas `3/5/6`, основной VLESS URI, web-admin и routing policy не меняются.
- Переносимый console/cloud-init bundle без выбранного SSH target продолжает определять платформу на самом сервере. Для штатной SSH-установки отсутствие platform facts или expected manifest является ошибкой, а не поводом предположить Ubuntu.

## [0.22.4] - 2026-09-05

### Fixed

- Потеря SSH после запуска автономного install/remove/rollback больше не даёт контроллеру права удалить рабочий каталог ещё работающего установщика. Состояние сверяется с systemd и transaction; ручной rollback использует общий путь подтверждения восстановления.
- Очистка Docker audit ограничена ресурсами своего checkout и хоста с подтверждённо завершённым процессом-владельцем. Чужие, неатрибутированные и явно сохранённые ресурсы не удаляются; очистка использует Docker ID, а не повторно используемые имена.
- Генерация клиентских файлов сначала создаёт и проверяет новый комплект. Ошибка записи восстанавливает прежние файлы; если восстановление невозможно, резервная копия сохраняется с указанием пути. Публикация защищена от одновременного запуска.
- Windows route helper работает только с адресами серверов, не превращает дополнительные CIDR-исключения в `/32` и не удаляет сторонние маршруты. Удаление ограничено совпадающей записью своего журнала и текущей загрузкой компьютера. Helper не запускается установщиком автоматически.
- Новые dual-установки получают отдельный случайный пароль web-admin. Отсутствующий пароль больше не заменяется общим `password`; существующий пароль при обновлении не меняется. Запись хеша использует общий атомарный writer; некорректный хеш и Unicode-логин обрабатываются без необработанного исключения.

### Compatibility

- Обновление только с `0.22.3`, fresh install и reinstall `0.22.4`. Schemas `3/5/6`, основной VLESS URI и сетевая routing policy не меняются.
- Web-admin сохраняется. Публичный HTTP-доступ сам по себе не шифрует пароль; защищённый доступ через штатный SSH описан в `docs/ADMIN.md`. Обновление не переключает пользователя на новый адрес панели и не сбрасывает его пароль.

## [0.22.3] - 2026-09-05

### Fixed

- Исправлена оставшаяся ветка частых межсерверных переключений: единичная потеря quality-probe больше не переключает исправный transport. Нужны две независимые пары проверок с существующим интервалом; soft degradation не обходит preferred retry. Быстрая реакция на подтверждённый hard failure сохраняется. После переключения сохраняется время последней quality-пробы, чтобы следующий цикл не запускал немедленный обратный переход.
- Успешная проверка текущего пути больше не стирает задержку повторной активации неисправного alternate. История неудачной активации ограничена существующим сроком хранения; новые публичные параметры не добавлены.
- Public VLESS verify вычитает baseline событий отдельных потоков и проверяет source внешнего runner. Старый неизменный snapshot или события другого source больше не подтверждают новый запуск. Это не заменяет application-level проверку Telegram и не доказывает доступность каждого его адреса.
- `diagnose path` сохраняет ошибки сбора валидным JSON и возвращает код `1`, если не удалось получить обязательный отчёт. Успешные отчёты других узлов сохраняются.

### Removed

- Удалён validator расширенной metadata для отсутствующих transition adapters. Предыдущий поддерживаемый manifest использует тот же канонический переход `from/to`; runtime-миграции не добавлены.

### Compatibility

- Fresh install и reinstall `0.22.3` поддерживаются; обновление только с `0.22.2`. Config/state, manifest/install-plan и diagnostics сохраняют schemas `3/5/6`.
- Основной `vless-uri.txt`, routing policy, web-admin, WireGuard MTU и настройки действующего локального клиента не изменены. Неуспешные соединения к недоступному с exit адресу не перенаправляются в RU direct или другой Telegram DC.
...
- Rollback использует ту же server-transaction reconciliation, что install/reinstall. Ожидаемый разрыв управляющего SSH при restart больше не объявляет откат неуспешным: результат принимается только после повторного preflight и точной проверки previous release, сервисов и полного VLESS-пути.
...
- Post-switch proof межсерверного selector больше не использует одиночный трёхсекундный ICMP/MTU тест сразу после сброса relay association. Agent даёт внутреннему WireGuard короткое bounded окно сходимости и подтверждает новый underlay несколькими реальными DNS-over-TCP обменами через `wg0`; rollback проверяется тем же полным dataplane-контрактом.
- Истёкшая ошибка selector activation больше не переносится во все последующие healthy snapshots. Активный backoff остаётся видимым, а успешное переключение очищает его без потери исторической записи в journald.
...
- Основной `vless-uri.txt`, Xray/Reality, routing policy, DNS, WireGuard MTU, web-admin и schema versions не изменены. Окно обновления: `0.21.7..0.21.8`.

## [0.21.7] - 2026-08-30

### Fixed

- Failure-injection lab приведён к transport-state schema `13`: два bounded DNS-over-TCP отказа выбранного dataplane и подтверждённый ответ alternate являются одним полностью подтверждённым циклом и должны переключать selector сразу. Lab дополнительно проверяет, что следующий цикл сохраняет здоровый fallback, активный TCP stream завершается одним HTTP-запросом, а восстановленный preferred возвращается по backoff policy.

### Compatibility

- Runtime и клиентские артефакты идентичны `0.21.6`; окно обновления `0.21.6..0.21.7`.

## [0.21.6] - 2026-08-30

### Fixed

- Автономная remote install transaction теперь создаёт `/var/log/vpn-stack` точной POSIX-константой. В `0.21.5` Windows controller ошибочно вычислял родительский каталог через `WindowsPath`, поэтому transient-service не запускался и сервер безопасно оставался на предыдущем release.

### Compatibility

- Dataplane, клиентские артефакты и schema versions не изменены. Окно `0.21.4..0.21.6` позволяет обновить production напрямую с не затронутого `0.21.4`, а также принять `0.21.5`, если он был установлен с Linux controller.

## [0.21.5] - 2026-08-30

### Fixed

- Interserver failover больше не принимает одиночный ICMP за отказ рабочего пути. Liveness теперь дважды проверяет реальный TCP DNS-обмен через внутренний `wg0`; один потерянный ответ сохраняет текущий underlay, а два отказа вместе с доказанным ответом того же foreign DNS relay через alternate разрешают переключение в одном watcher-цикле. Переключение по одной только задержке или packet loss по-прежнему запрещено.
- ICMP quality sample отделён от liveness: loss сохраняется как `degraded`, но не сбрасывает relay и не создаёт разрыв активных потоков. Последний sample действует до следующего измерения, поэтому status не скрывает деградацию между 15-секундными циклами.
- Строки sing-box `use of closed network connection` классифицируются как завершённый клиентом запрос `client_reset_eof`, а не как неизвестная серверная DNS-ошибка.
- Установка запускается автономным transient-service systemd. Upload и preflight остаются управляемыми локальным CLI, но начавшаяся server transaction продолжает выполняться при обрыве SSH/чата; полный stdout/stderr перезаписывается в закрытый правами `0600` файл `/var/log/vpn-stack/install.log`, а временный bundle удаляется server-side trap.
...
- Основной `vless-uri.txt`, Reality/Xray front, routing policy, DNS rules, MTU, web-admin и публичные env-параметры не изменены. Внутренняя transport-state schema поднята до `13`; старое ephemeral-состояние не мигрируется, а безопасно собирается заново. Окно обновления: `0.21.4..0.21.5`.
...
- Устранена подтверждённая причина периодических обрывов на российском gateway: при 710 МБ RAM без swap ядро убило `sing-box` по OOM 21 августа. Router теперь запускается с вычисляемым от фактической памяти `GOMEMLIMIT`, оставляющим системе не менее 384 МБ, а узел с RAM менее 1 ГБ до старта dataplane получает закрытый правами `0600` резерв swap объёмом 512 МБ. Отсутствующий memory budget или обязательный резерв является ошибкой acceptance, а не скрытым предупреждением.
- Snapshot и health сохраняют текущую/пиковую память `sing-box`, автоматические рестарты systemd и kernel OOM с раздельными окнами. Поэтому восстановившийся за три секунды процесс больше не превращает реальное падение в ложный `verified` без evidence.
- Проверка полного публичного VLESS/Reality-пути теперь требует по три успешных first-load запроса к VK, Google static и GitHub. Это проверяет `direct-ru` и `to-foreign` через основной клиентский URI, а не только локальный SOCKS router.
- Compatibility validator учитывает точный additive delta install contract при переходе с `0.21.2`. Раньше Docker fixture копировал новый artifact/package set под старой версией и не замечал, что реальный предыдущий bundle корректно не содержит ещё не существовавшие файлы; production transaction безопасно откатывал такую попытку до переключения gateway.
- Binary drift сверяет фактический `/proc/<MainPID>/exe`, а не первый executable из systemd `ExecStart`. Поэтому прозрачный Python launcher, устанавливающий `GOMEMLIMIT` и заменяющий себя через `execve`, не создаёт ложный `wrong-exec`; при этом hash бинарника и реальный запущенный inode по-прежнему обязаны совпасть.
- Проверка swap опирается на активный managed device и допускает служебные 4 КиБ, которые ядро не включает в `SwapTotal`; ровный файл 512 МБ больше не помечается как отсутствующий. Код возврата `journalctl=1` при отсутствии OOM matches также считается пустым результатом, а не ошибкой collector.
- Acceptance private/fake reject больше не зависит от уровня успешных runtime-логов: внешний probe подтверждает быстрый EOF, а target-side agent независимо сверяет неизменённый installed config и порядок reject guard перед catch-all. Возврат данных, медленный таймаут, неизвестная цель или config drift по-прежнему дают `failed`/`inconclusive`.
- Storage snapshot считает реально выделенные filesystem blocks, поэтому sparse-файлы journald больше не завышают расход диска. Отдельно показываются ограниченные журналы безопасности и стабильные APT indexes/cache; package lists сохраняются для unattended security updates и не выдаются за накопительный мусор.
- First-load evidence для каждого запроса теперь содержит DNS/connect/TLS/TTFB/total timings, remote IP, код `curl`, краткую ошибку и последнюю завершённую фазу. Скрытых retries нет: первый сорванный запрос всё ещё проваливает основной VLESS gate, но следующий инцидент можно локализовать без шумного `info`-лога всех соединений.
- Post-cutover gate отделён от эксплуатационного verdict: потери активного постороннего client source больше не откатывают полностью прошедший независимый VLESS/HY2 release. Исключение допускается только для `client_specific` TCP-front degradation при подтверждённых server path, host integrity, drift-free manifest и всех release probes; обычный `verify live` продолжает возвращать `degraded`.

### Changed

- Штатный уровень журнала `sing-box` снижен с `info` до `warn`: production больше не записывает каждый успешный поток. Точное прежнее default-значение автоматически мигрируется только при обновлении с `0.21.2`; явно заданный operator override сохраняется.
- После успешной acceptance установка хранит только immutable `current` и `previous` release trees. Удаление ограничено прямыми дочерними каталогами `/etc/vpn-stack/releases`; rollback меняет ссылки атомарно и снова проходит acceptance.
- Добавлена bounded-очистка источников роста диска: APT cache очищается после установки, `btmp` ротируется при 64 МБ с двумя сжатыми копиями, journald остаётся ограничен 256 МБ и 14 днями. Snapshot показывает свободное место, объём release store, journal и security logs; 85% заполнения даёт `degraded`, 95% — `failed`.
- Реализован ранее только документированный лимит transaction backups: перед созданием нового снимка сохраняются текущая rollback-точка и свежие revisions, поэтому после создания их не больше десяти. Status отдельно показывает revisions, baseline и управляемый swap; APT раз в неделю выполняет штатный autoclean устаревших package archives.
- Успешное обновление удаляет оставшийся от генератора до `0.20.0` каталог `backups/snapshots`: он больше не участвует в rollback и не пополняется текущим installer. Удаление ограничено точным project-owned путём; действующие `baseline` и `revisions` сохраняются.
- Фоновая очистка выполняется после health и не может отменить сетевую проверку при собственной ошибке. Глубокая очистка запускается только после успешной транзакционной установки.
...
- VLESS URI, routing policy, DNS, MTU, transport selector и web-admin не изменены. Окно обновления ограничено `0.21.2..0.21.3`; config/state schema `3`, manifest/install-plan schema `4` и diagnostics schema `5` сохранены.

## [0.21.2] - 2026-08-19

### Fixed

- Health больше не переносит `client_specific` из предыдущего двухминутного front-интервала в следующий здоровый цикл. Новый интервал полностью заменяет прошлое наблюдение, поэтому единичная клиентская потеря не оставляет ложный `degraded` ещё на один цикл и не запускает recovery.
- В evidence деградации публичного TCP-фронта сохраняются PMTU, MSS, RTT, RTO, congestion window, delivery rate и reordering именно проблемного сокета. `vpn diagnose client` также выводит MSS рядом с PMTU; последующий инцидент можно отличить по фактам от MTU black hole, packet loss и медленного удалённого peer.
- Renderer удаляет устаревшие экспериментальные `sing-box-adaptive.json` и `live-xray-smoke.json`, которые больше не входят в текущую модель профилей. Каталог не пересоздаётся целиком: пользовательские заметки и неизвестные файлы сохраняются.

### Diagnosed

- Сбой мобильных клиентов в новой Wi-Fi сети локализован вне серверного dataplane: Tenda с DHCP включённым раздавал DNS `192.168.0.1`, но его DNS-прокси не отвечал, тогда как оба upstream DNS провайдера и `1.1.1.1` отвечали. Компьютер работал, потому что уже использовал прямые `1.1.1.1/1.0.0.1`; смена DNS на телефоне устранила различие.
- Изолированная проверка из той же физической сети прошла через оба публичных профиля: VLESS/Reality и Hysteria2 открыли все тестовые домены и изображения. Маршрут до VPS имеет PMTU `1480`, ICMP Packet Too Big проходит, активный VLESS TCP автоматически выбрал меньший MSS и передал тестовый поток без признаков MTU black hole.

### Compatibility

- VLESS URI, routing policy, WireGuard MTU, DNS сервера на VPN-узлах и transport selector не изменены. Окно обновления ограничено `0.21.1..0.21.2`; config/state schema `3`, manifest/install-plan schema `4` и diagnostics schema `5` сохранены.
...
- Transport watcher сохраняет быстрый одиночный liveness каждые `2s` и только после его успеха раз в `15s` выполняет четыре ICMP-пакета по `1200` байт через фактически выбранный overlay. Quality-loss переводит путь в `suspect`, но переключение требует следующего свежего liveness failure; полный отказ не ждёт длинный quality deadline. Затем проверяется альтернативный underlay и меняется selector без перезапуска Xray, sing-box или `wg0`.
- После отказа основного пути его возврат откладывается на `60s` с экспоненциальным ростом до `15min`. После `10min` устойчивой работы история отказа сбрасывается. Это сохраняет автоматический возврат к прямому WireGuard, но исключает постоянное переключение на живой, однако перегруженный канал.
- Snapshot хранит последний quality-probe отдельно от двухсекундного liveness и показывает packet loss, RTT и срок следующей попытки. Transport state schema поднята до `12`; удалены неиспользуемые поля совместимости старой latency-схемы.

### Verified

- Основной VLESS URI, Reality/Xray front, routing policy, DNS, MTU `1360`, web-admin и локальные клиентские настройки не изменены. Периодические throughput-тесты в health не добавлены.
- После production cutover текущий inner-WireGuard path передал одним TCP-потоком `116 Мбит/с` RU -> exit и `84.5 Мбит/с` exit -> RU, четырьмя потоками - `144/99 Мбит/с`. Полный внешний тест через неизменный `vless-uri.txt` дал `9/9` успешных first-load запросов, `19.0 Мбит/с` sustained и `41.6 Мбит/с` peak capacity без server-side DNS/route timeout.
- Окно обновления ограничено `0.21.0..0.21.1`; schema adapters и ветки совместимости для `0.20.x` не добавлены.

## [0.21.0] - 2026-08-17

- SSH-пароль стал первым вариантом авторизации в интерактивной установке; ручной ввод скрыт и выполняется только в памяти процесса.
- Добавлено необязательное безопасное сохранение проверенного SSH-пароля в Windows Credential Manager или Linux Secret Service без файлового fallback и без записи секретов в state, deployment env или логи.
- Текущая версия показана в главном меню и доступна через `--version`; после установки двойной схемы выводится адрес web-admin.
- Пользовательская документация перестроена вокруг интерактивного интерфейса, дополнена справочником команд и отдельными примерами путей для Windows и Linux.
- Окно обновления ограничено версиями `0.20.2..0.21.0`; compatibility runtime для более старых релизов отсутствует.
...
- Linux release gate снова является исполняемым контрактом: dev-зависимость `cryptography` объявлена явно, POSIX-тест блокировки проверяет фактическую сериализацию `flock -w 60` через детерминированный handshake, nft-тест больше не требует удаления чужих legacy `nat`-таблиц, а transport unit не пишет в `/var/lib/vpn-stack`. При падении полный каталог audit evidence сохраняется как GitHub artifact; устранены ложные отказы публикации, из-за которых теги после `0.16.2` не превращались в GitHub Releases.
...
- В интерфейс выводится только первая ограниченная строка ошибки. Python traceback, SSH stderr и PowerShell detail сохраняются в `out/logs/runtime`, включая отдельный архивный error log.
- Имя ключа без каталога разрешается относительно стандартного `~/.ssh`; полный путь и явный относительный путь продолжают поддерживаться.
...
- Устранён подтверждённый цикл деградации межсерверного transport: watcher принимал задержку ответа foreign DNS relay свыше `800ms` за отказ WireGuard, после двух ускоренных проверок пытался сменить endpoint и при неудачном MTU proof повторял switch/rollback каждые несколько секунд. На live-сервере эти циклы точно совпадали с сериями DNS, domain и IP-literal timeout.
- Transport-liveness теперь проверяется отдельным малым ICMP внутри фактического kernel WireGuard overlay и не зависит от resolver, CDN либо Internet endpoint. DNS остаётся самостоятельной acceptance/health-проверкой и больше не управляет underlay.
- Cold fallback считается доступным только после получения реального SSH banner от приватного адреса foreign через соответствующий relay. Локальный успешный SOCKS5 CONNECT больше не является достаточным доказательством: в production он завершался за `1-4ms`, тогда как реальный ответ foreign приходил через `53-131ms`.
- Смена endpoint больше не закрывает рабочую relay association до proof нового пути. Активация bounded-повторяет малый liveness и MTU probe; при ошибке сначала возвращает старый endpoint, проверяет его и только затем очищает неудачную association.
- Неуспешная активация fallback открывает внутренний bounded circuit breaker с экспоненциальным интервалом `30-300s`. Watcher больше не разрушает восстановленный dataplane повторными switch/rollback в одном инциденте; один здоровый цикл выбранного пути сбрасывает активную блокировку, поэтому следующий независимый инцидент снова обрабатывается сразу.

### Changed

- Transport state schema обновлена до `9` и сохраняет последнюю ошибку активации отдельно от текущего verdict. Probe-cycle снова выполняется с обычным интервалом `2s`, а не ускоряется до `250ms` в `suspect`.
- Основной `vless-uri.txt`, Reality/Xray front, routing policy, WireGuard MTU `1360`, web-admin и запрет RU-direct fallback не изменены.
...
- Reinstall больше не перезапускает исправный `systemd-resolved`, если managed resolver config побайтно совпадает с предыдущим release и TCP/UDP stub уже слушает `127.0.0.53:53`. Раньше restart выполнялся при каждом релизе, и ещё работающий sing-box успевал получить краткий `connection refused` до последующей readiness-проверки.
- Release acceptance теперь явно отклоняет активацию, если в полном окне `since_release` появился отказ локального resolver `127.0.0.53:53`; такой startup race больше не может скрыться за успешными более поздними probes.

### Changed

- При первой установке, изменении resolver config либо неготовом stub сервис по-прежнему перезапускается и bounded-проверяется. Dataplane, Reality target, маршрутизация, web-admin и клиентские артефакты не изменены.
...
- Автоматический transport watcher больше не принимает доступность одного внешнего Cloudflare URL за исправность межсерверного dataplane. Активный путь проверяется локальным DNS wire-запросом `localhost A` через фактический kernel WireGuard overlay и установленный foreign DNS relay; решение не зависит от внешнего DNS, CDN или конкретного сайта.
- Холодный underlay проверяется отдельным SOCKS5 CONNECT через соответствующий relay до приватного адреса foreign-сервера. Два свежих отказа реального overlay и два подтверждения альтернативы переключают только локальный endpoint `wg0`; открытые клиентские TCP-потоки, Xray и sing-box не перезапускаются.
- Исправлено бессрочное закрепление на Hysteria2 после краткой деградации. При исправном fallback предпочтительный userspace WireGuard проверяется раз в `10s` и после трёх независимых успешных циклов возвращается автоматически; RTT и скорость не являются основанием для переключения.
- Post-switch proof больше не зависит от внешнего HTTP endpoint: он требует MTU-safe ping, двусторонний рост WireGuard counters и валидный ответ реального DNS relay. Неуспешное доказательство по-прежнему атомарно возвращает предыдущий endpoint и проверяет rollback.
- Текущий verdict публичного Xray front больше не наследует lifetime retransmit/RTT от давно простаивающего `ESTAB`. Исторические counters остаются телеметрией, а degradation формируется только из активного сокета за последние `30s` или монотонной дельты свежего health-интервала.
- Structured log collector сохраняет problem-only основной запрос, но bounded-добавляет inbound/DNS INFO-контекст совпадающих sing-box event ID. IP назначения в timeout теперь коррелируется с исходным доменом без полного сканирования INFO-журнала.
- Удалена устаревшая lab-подмена внешнего healthcheck URL: production и failure-injection lab используют один локальный алгоритм; lab доказывает сохранение одного TCP-потока при failover и трёхцикловое возвращение на восстановленный WG.

### Changed

- Основной `vless-uri.txt`, routing policy, WireGuard MTU `1360`, публичные Xray/Hysteria2 входы, web-admin и запрет RU-direct fallback не изменены. Новых env/timeout-настроек не добавлено.
...
- Диагностика multiplex больше не подменяет отсутствие активного outer socket выводом `not_detected`. Статусы разделены на доказанный `detected`, `not_observed` в наблюдаемом активном окне и `inconclusive`, когда активного потока уже нет.

### Changed

- Серверный dataplane, клиентские профили, основной VLESS URI и web-admin не изменены; патч обновляет только установленный диагностический агент и operator output.
