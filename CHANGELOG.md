...
- Transport watcher сохраняет быстрый одиночный liveness каждые `2s` и раз в `15s` выполняет четыре ICMP-пакета по `1200` байт через фактически выбранный overlay. Любая измеренная потеря должна повториться в двух свежих циклах; только после этого проверяется альтернативный underlay и меняется selector без перезапуска Xray, sing-box или `wg0`.
