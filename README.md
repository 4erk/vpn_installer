...
Для выбора узла используется только `--node gateway|exit|all`. Параметр `--role` и старые имена ролей не поддерживаются.

## Обновление 0.20.2

Установщик `0.20.2` допускает только три исходных состояния:

| Исходное состояние | Результат |
| --- | --- |
| VPN Installer ещё не установлен | Чистая установка |
| Установлен `0.20.1` | Поддерживаемое обновление на `0.20.2` без смены схем |
| Установлен `0.20.2` | Повторная установка той же версии |

Минимальная совместимая установленная версия — `0.20.1`, максимальная — `0.20.2`. Любая другая версия отклоняется до изменения управляемых файлов. Для её удаления или полной очистки запусти `vpn.cmd`/`vpn.sh` именно из Git-тега установленной версии, выполни доступную в ней команду `remove` или `purge`, а затем установи `0.20.2` с нуля. Не используй новый установщик для удаления неподдерживаемого runtime.

Текущие форматы не изменились: config/state schema `3`, manifest/install-plan schema `4`, diagnostics schema `5`. Активных migration adapters нет. Подробности: [совместимость версий](./docs/DEPRECATIONS.md).
...
- [Совместимость версий](./docs/DEPRECATIONS.md)
