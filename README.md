...
После установки на `российском сервере` поднимается простой web admin для изменения server-side исключений без ручного редактирования env-файлов. По умолчанию он слушает только `127.0.0.1:11333`, поэтому безопасный доступ с компьютера оператора такой:
...
Если нужен прямой доступ к `http://<server>:11333`, задай в deployment env:

```env
ADMIN_WEB_BIND="0.0.0.0"
ADMIN_WEB_ALLOWED_CIDR="<твой-публичный-ip>/32"
ADMIN_WEB_USERNAME="..."
ADMIN_WEB_PASSWORD="..."
```

Не открывай этот порт на весь интернет: TLS там нет, а `user/password` при публичном bind запрещён.
