...
Между серверами foreign-трафик идёт внутри одного стабильного kernel WireGuard overlay. Его локальный endpoint может без перезапуска интерфейса переключаться между userspace WireGuard и Hysteria2/QUIC до того же foreign egress. Пока выбранный underlay исправен, он остаётся sticky и не переключается из-за колебаний RTT. После подтверждённого отказа agent двумя свежими bounded-probe доказывает альтернативный путь, закрывает только association отказавшего relay и переносит endpoint; Xray, sing-box, kernel WG и уже открытые TCP-потоки не перезапускаются. Оба underlay заканчиваются на одном зарубежном VPS, поэтому это transport redundancy, а не второй независимый egress. Публичный клиентский контракт остаётся тем же VLESS/Reality URI; дополнительно выпускаются mux-free VLESS JSON и стандартный `hysteria2://` URI.
