...
Между серверами foreign-трафик идёт внутри одного стабильного kernel WireGuard overlay. Agent проверяет transport малым ICMP внутри фактического overlay; resolver, CDN и отдельный Internet endpoint не участвуют в выборе underlay. После двух свежих отказов он требует реальный SSH banner от приватного адреса foreign через холодный relay, переносит локальный endpoint `wg0` и только после успешных liveness/MTU proofs закрывает старую association. Неуспешная активация возвращает уже готовый старый путь и включает bounded circuit breaker вместо повторных switch/rollback. Xray, sing-box, интерфейс `wg0` и уже открытые TCP-потоки не перезапускаются. При восстановлении предпочтительный userspace WireGuard возвращается после трёх успешных проверок с интервалом `10s`; колебания RTT и скорости маршрутом не управляют. Оба underlay заканчиваются на одном зарубежном VPS, поэтому это transport redundancy, а не второй независимый egress. Публичный клиентский контракт остаётся тем же VLESS/Reality URI; дополнительно выпускаются mux-free VLESS JSON и стандартный `hysteria2://` URI.
...
Release acceptance по умолчанию измеряет полный VLESS path в течение `30s`; для более длинного ручного окна укажи `60s`. Runner по кругу использует два Hetzner endpoint и независимый Cloudflare payload. Hard gate требует sustained goodput не ниже `10 Mbit/s`, минимум два реально передающих источника, полный интервал без transfer failures и без пауз прогресса длиннее budget. Лучший peak сравнивается с reference `50 Mbit/s` только как диагностика: скорость отдельного CDN не вызывает rollback исправного VPN. Global lock исключает конкурирующие запуски, а controller lease и target-side deadline завершают process group при потере управляющей команды. Для функциональной проверки без нагрузки укажи `--throughput-seconds 0`:
