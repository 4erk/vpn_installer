...
Между серверами foreign-трафик идёт через адаптивную пару Hysteria2/QUIC и WireGuard. Supervisor проверяет выбранный путь; резерв включается в проверку только после первого отказа или пока он уже выбран. Подтверждённая смена применяется только к новым соединениям, synthetic RTT и общесистемные counters маршрут не меняют. Оба transport заканчиваются на одном зарубежном VPS, поэтому это не подменяет второй независимый egress. Публичный клиентский контракт при этом остаётся тем же VLESS/Reality URI.
