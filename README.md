...
Между серверами foreign-трафик идёт внутри одного стабильного kernel WireGuard overlay. Agent проверяет фактический overlay локальным DNS wire-запросом через установленный foreign relay, поэтому автоматическое решение не зависит от внешнего DNS, CDN или отдельного сайта. После двух свежих отказов он доказывает холодный underlay SOCKS5 CONNECT до приватного адреса foreign-сервера, закрывает только association отказавшего relay и переносит локальный endpoint `wg0`. Xray, sing-box, интерфейс `wg0` и уже открытые TCP-потоки не перезапускаются. При восстановлении предпочтительный userspace WireGuard возвращается после трёх успешных проверок с интервалом `10s`; колебания RTT и скорости маршрутом не управляют. Оба underlay заканчиваются на одном зарубежном VPS, поэтому это transport redundancy, а не второй независимый egress. Публичный клиентский контракт остаётся тем же VLESS/Reality URI; дополнительно выпускаются mux-free VLESS JSON и стандартный `hysteria2://` URI.
