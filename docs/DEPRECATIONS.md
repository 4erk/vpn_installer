...
Релиз `0.21.6` записывает только текущие форматы:
...
Manifest каждого узла объявляет `installed_min=0.21.4` и `installed_max=0.21.6`.

| Состояние сервера | Допустимое действие |
| --- | --- |
| Управляемой установки нет | Fresh install |
| Установлен `0.21.4` или `0.21.5` | Обновление на `0.21.6` тем же current-schema validator |
| Установлен точный `0.21.6` | Same-version reinstall |
...
- `0.21.4`, `0.21.5` и `0.21.6` проходят один validator config/state `3`, manifest/install-plan `4` и diagnostics `5`; version-specific schema adapters отсутствуют.
...
Активных deprecated env-ключей, contract deltas, role aliases, readers старых схем и migration chains в `0.21.6` нет. Поддержка обновления с `0.21.3` завершена: новый релиз принимает только текущие `0.21.4`/`0.21.5` и не содержит version-specific adapters.
