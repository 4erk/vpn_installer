...
Релиз `0.21.4` записывает только текущие форматы:
...
Manifest каждого узла объявляет `installed_min=0.21.3` и `installed_max=0.21.4`.

| Состояние сервера | Допустимое действие |
| --- | --- |
| Управляемой установки нет | Fresh install |
| Установлен точный `0.21.3` | Обновление на `0.21.4` тем же current-schema validator |
| Установлен точный `0.21.4` | Same-version reinstall |
...
- `0.21.3` и `0.21.4` проходят один validator config/state `3`, manifest/install-plan `4` и diagnostics `5`; version-specific schema adapters отсутствуют.
...
Активных deprecated env-ключей, contract deltas, role aliases, readers старых схем и migration chains в `0.21.4` нет. Поддержка обновления с `0.21.2`, миграция его прежнего log-level default и одноразовая очистка старого snapshot-store завершены в `0.21.3` и удалены из текущего runtime.
