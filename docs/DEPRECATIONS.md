...
Релиз `0.20.2` записывает только текущие форматы:
...
Manifest каждого узла объявляет `installed_min=0.20.1` и `installed_max=0.20.2`.

| Состояние сервера | Допустимое действие |
| --- | --- |
| Управляемой установки нет | Fresh install |
| Установлен точный `0.20.1` | Обновление на `0.20.2` тем же current-schema validator |
| Установлен точный `0.20.2` | Same-version reinstall |
| Любая другая или нераспознаваемая версия | Отказ до изменения managed runtime |

Для неподдерживаемого релиза используй `vpn.cmd` или `vpn.sh` из совпадающего Git-тега. Выполни доступную в той версии команду `remove`/`purge`, затем установи текущий релиз с нуля. Новый установщик не угадывает ownership или формат чужого поколения.

## Инварианты

- Проверка установленной версии выполняется до package install, upload, transaction snapshot и переключения `current`.
- `0.20.1` и `0.20.2` проходят один validator config/state `3`, manifest/install-plan `4` и diagnostics `5`; version-specific schema adapters отсутствуют.
...
Активных deprecated env-ключей, role aliases, readers старых схем и запланированных schema migrations в `0.20.2` нет.
