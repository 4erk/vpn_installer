...
Релиз `0.21.1` записывает только текущие форматы:
...
Manifest каждого узла объявляет `installed_min=0.21.0` и `installed_max=0.21.1`.

| Состояние сервера | Допустимое действие |
| --- | --- |
| Управляемой установки нет | Fresh install |
| Установлен точный `0.21.0` | Обновление на `0.21.1` тем же current-schema validator |
| Установлен точный `0.21.1` | Same-version reinstall |
...
- `0.21.0` и `0.21.1` проходят один validator config/state `3`, manifest/install-plan `4` и diagnostics `5`; version-specific schema adapters отсутствуют.
...
Активных deprecated env-ключей, role aliases, readers старых схем и запланированных schema migrations в `0.21.1` нет. Поддержка обновления с `0.20.2` завершена; её код в текущий runtime не переносится.
