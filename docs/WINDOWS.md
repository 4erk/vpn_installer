...
## Совместимость 0.20.2

`0.20.2` устанавливается с нуля, обновляет только `0.20.1` и допускает повторную установку `0.20.2`. Окно установленных версий: минимум `0.20.1`, максимум `0.20.2`. Для любой неподдерживаемой версии используй `vpn.cmd` из совпадающего Git-тега, удали установку доступной в той версии командой `remove`/`purge`, затем выполни чистую установку текущего релиза.
