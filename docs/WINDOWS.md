...
Пароль принимает встроенный Python backend, без запуска `ssh.exe`. Он используется только в памяти процесса и не сохраняется в `state` или deployment env. Для non-interactive запуска пароль передаётся через `VPN_GATEWAY_SSH_PASSWORD`, `VPN_EXIT_SSH_PASSWORD` либо общий `VPN_SSH_PASSWORD`.
...
Веб-панель существует только у двойной установки. Сначала подключись к её публичному VPN ingress, затем запроси адрес gateway:
...
## Совместимость 0.20.1

`0.20.1` устанавливается с нуля, обновляет только `0.20.0` и допускает повторную установку `0.20.1`. Окно установленных версий: минимум `0.20.0`, максимум `0.20.1`. Для любой неподдерживаемой версии используй `vpn.cmd` из совпадающего Git-тега, удали установку доступной в той версии командой `remove`/`purge`, затем выполни чистую установку текущего релиза.

Все команды `0.20.1` принимают только `--node gateway|exit|all`; `--role` удалён.
