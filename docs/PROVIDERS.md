...
Ниже — практический shortlist для одиночной схемы и для контура `клиент -> российский сервер -> зарубежный сервер`.
...
Эта пара использовалась автором лично. Названия ссылок не содержат referral ID, но сами URL включают referral-параметры. Рекомендация не отменяет проверку конкретного тарифа: нужен публичный IPv4, `x86_64`, `systemd`, ОС из [поддерживаемой матрицы](./PLATFORMS.md), root/sudo и SSH. Перед установкой `ufw` и `firewalld` должны быть неактивны. Для `single` достаточно публичного VPN ingress на TCP/UDP `443`; WireGuard, межсерверные порты и web-admin не требуются. Для `dual` дополнительно нужны межсерверный трафик между gateway/exit и public web-admin port на gateway, доступ к которому ограничивают app-owned таблицы `nftables`.
...
- российский `IPv4`
- ОС из [поддерживаемой Linux-матрицы](./PLATFORMS.md)
- нормальный `SSH`
