...
- `RU_IPV6_POLICY=to-foreign` по умолчанию отправляет IPv6 literals через зарубежный сервер: это сохраняет доступность сайтов для клиентов, которые всё равно присылают на сервер IPv6-адрес вместо домена
