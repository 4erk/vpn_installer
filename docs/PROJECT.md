...
Target-side render не объединяет `node.env` с общими defaults и не генерирует ключи. Он принимает только точную `CONFIG_SCHEMA=3` проекцию capability, отклоняет неизвестные поля и cross-node secrets, затем сверяет payload с manifest/install-plan. Установленный `0.22.2` имеет те же schemas и проходит общий текущий validator без отдельного adapter.
...
`0.22.3` поддерживает fresh install, обновление только с `0.22.2` и повторную установку `0.22.3`. Manifest объявляет `installed_min=0.22.2`, `installed_max=0.22.3`. Неподдерживаемый установленный релиз отклоняется до managed transaction; удалить его нужно `.\vpn.cmd` на Windows или `./vpn.sh` на Linux из совпадающего Git-тега, после чего выполняется fresh install.
...
- Transport redundancy не является egress redundancy: оба пути заканчиваются на одном foreign VPS. Snapshot поэтому отдельно выводит `redundancy.transport` и `redundancy.egress`; второго независимого foreign сервера в этой topology нет.
- Один статический VLESS URI не может сменить transport и поэтому остаётся совместимым TCP-контрактом. Mux не кодируется стандартным URI и должен оставаться выключенным в клиенте; JSON фиксирует это явно. Дополнительный Hysteria2 URI даёт отдельный QUIC-транспорт для сетей с потерями на TCP-пути. Это не per-request retry: произвольный частично переданный запрос нельзя безопасно воспроизвести на другом transport. Замена RAW/Reality/Vision на XHTTP требует отдельной проверки совместимости и задержек; описаны случаи compatibility/latency regressions ([Xray-core #5332](https://github.com/XTLS/Xray-core/issues/5332)).
