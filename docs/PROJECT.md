...
- Agent выполняет параллельный bounded-probe обоих underlay каждые `2s` с budget `1200ms`. Отказ выбранного пути должен подтвердиться дважды; latency-переключение требует преимущества более `30ms` три цикла подряд. Перед сменой endpoint закрывается только старая UDP-association соответствующего relay через локальный Clash API, после чего `wg set ... endpoint` атомарно переводит overlay. Ни Xray, ни sing-box router, ни `wg0` не перезапускаются.
- Внутренний overlay использует `PersistentKeepalive=1`, чтобы foreign peer выполнил roaming даже при замершем TCP без исходящих ACK. Внешний userspace WireGuard имеет отдельную детерминированную identity и отдельный peer на том же закрытом foreign listener; новых публичных портов нет. Failure-injection lab требует завершения одного 5 MiB TCP-потока без повторного HTTP-запроса после потери прямого underlay.
