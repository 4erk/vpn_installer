...
- Agent каждые `2s` проверяет фактический kernel `wg0` overlay двумя bounded DNS-over-TCP обменами с управляемым foreign relay. Первый успешный ответ подтверждает liveness; только два отказа подтверждают недоступность выбранного dataplane. После успешного liveness раз в `15s` ICMP sample из двадцати пакетов по `1200` байт измеряет loss/RTT. Свежая потеря на preferred WireGuard разрешает переход только после отдельного успешного ответа alternate relay; отсутствие такого доказательства оставляет маршрут неизменным. Альтернативный raw underlay обязан вернуть синтетический `localhost A` по UDP от того же foreign relay `10.75.0.2:1053` через свой локальный SOCKS endpoint с общим budget `1200ms`, поэтому решение не зависит от внешнего DNS, CDN, SSH или публичного health URL. На fallback возврат к WireGuard дополнительно требует чистой серии из восьми probe в общем окне `2400ms` и трёх последовательных recovery confirmations; частичная потеря увеличивает backoff `60..900s`. После `10min` устойчивой работы backoff сбрасывается. Постоянных throughput-тестов и переключения по одному RTT sample нет.
