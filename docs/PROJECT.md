...
- Agent каждые `2s` проверяет фактический kernel `wg0` overlay малым ICMP, а только после успешного liveness раз в `15s` измеряет потерю четырьмя пакетами по `1200` байт через уже выбранный путь. Quality-loss создаёт `suspect`, но selector меняется лишь после следующего свежего liveness failure; полный отказ поэтому не ждёт quality deadline. Альтернативный raw underlay затем обязан вернуть синтетический `localhost A` по UDP от управляемого foreign relay `10.75.0.2:1053` через свой локальный SOCKS endpoint с budget `1200ms`; запрос обслуживается foreign `systemd-resolved` без внешнего DNS, CDN, SSH или публичного health URL. Userspace WireGuard является штатным underlay для постоянного inner-WireGuard overlay, Hysteria2 — обходным каналом при блокировке или подтверждённой недоступности прямого UDP. Повторная попытка WireGuard выполняется через `60..900s`, а после `10min` устойчивой работы backoff сбрасывается. Постоянных throughput-тестов и переключения по одному RTT sample нет.
