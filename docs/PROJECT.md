...
- Выбранный interserver underlay проверяется двумя DNS-over-TCP обменами через точный внутренний WireGuard dataplane. Один сорванный обмен не создаёт failover; два отказа и успешный ответ того же DNS relay через alternate позволяют переключиться в одном цикле. Периодический ICMP quality sample остаётся soft degradation и не сбрасывает relay сам по себе.
...
- Agent каждые `2s` проверяет фактический kernel `wg0` overlay двумя bounded DNS-over-TCP обменами с управляемым foreign relay. Первый успешный ответ подтверждает liveness; только два отказа подтверждают недоступность выбранного dataplane. После успешного liveness раз в `15s` отдельный ICMP sample из четырёх пакетов по `1200` байт измеряет loss/RTT, но остаётся soft telemetry и не разрешает переключение. Альтернативный raw underlay обязан вернуть синтетический `localhost A` по UDP от того же foreign relay `10.75.0.2:1053` через свой локальный SOCKS endpoint с budget `1200ms`; поэтому решение не зависит от внешнего DNS, CDN, SSH или публичного health URL. Userspace WireGuard является штатным underlay для постоянного inner-WireGuard overlay, Hysteria2 — обходным каналом при блокировке или подтверждённой недоступности прямого UDP. Повторная попытка WireGuard выполняется через `60..900s`, а после `10min` устойчивой работы backoff сбрасывается. Постоянных throughput-тестов и переключения по одному RTT sample нет.
