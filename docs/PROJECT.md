...
- Foreign-трафик всегда идёт через один kernel WireGuard overlay с постоянными адресами и routing policy. Kernel peer смотрит на локальную relay-точку, поэтому смена внешнего transport не пересоздаёт интерфейс и не меняет сокеты приложений. Два статических relay доставляют зашифрованные overlay-пакеты через userspace WireGuard либо Hysteria2/QUIC; Hysteria2 не получает статических `up_mbps/down_mbps`, а его Salamander-ключ выводится из deployment secret ([sing-box Hysteria2](https://sing-box.sagernet.org/configuration/outbound/hysteria2/), [WireGuard roaming](https://www.wireguard.com/)).
- Agent каждые `2s` проверяет фактический dataplane локальным TCP DNS wire-запросом `localhost A` через kernel `wg0` и установленный foreign relay с budget `800ms`. При отказе альтернативный raw underlay проверяется SOCKS5 CONNECT до приватного foreign SSH endpoint с budget `1200ms`; внешний DNS, CDN и публичный health URL в автоматическом решении не участвуют. Два свежих failure/health цикла разрешают перенос endpoint. Пока fallback исправен, предпочтительный userspace WireGuard проверяется раз в `10s` и возвращается после трёх подтверждений. RTT/throughput не переключают маршрут. Перед сменой endpoint закрывается только старая UDP-association соответствующего relay через локальный Clash API, после чего `wg set ... endpoint` атомарно переводит overlay. Ни Xray, ни sing-box router, ни `wg0` не перезапускаются.
- Внутренний overlay использует `PersistentKeepalive=1`, чтобы foreign peer выполнил roaming даже при замершем TCP без исходящих ACK. Внешний userspace WireGuard имеет отдельную детерминированную identity и отдельный peer на том же закрытом foreign listener; новых публичных портов нет. Failure-injection lab требует завершения одного `7.5 MiB` TCP-потока без повторного HTTP-запроса после потери прямого underlay и проверяет обратный переход после восстановления.
...
`verify live --throughput-seconds 60` разделяет контракты по роли транспорта: 30-секундный uncapped burst канонического VLESS/TCP должен подтвердить не менее 50 Mbit/s, а резервный Hysteria2/QUIC - не менее 10 Mbit/s без обрывов. Capacity использует Hetzner FSN, Hetzner NBG и независимый Cloudflare payload; минимум два источника должны передать данные. Отказ одного CDN остаётся внешней telemetry, а не ложным отказом VPN. Затем bounded stability-фаза требует не менее 10 Mbit/s для обоих путей. Один global lock сериализует runner-профили; target-side deadline и SSH controller lease завершают process group при исчезновении локального процесса. Проверка не входит в health timer.
