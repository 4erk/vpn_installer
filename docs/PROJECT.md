...
- Для client-to-RU доступны два транспорта на одном номере порта: канонический VLESS/Reality использует TCP, Hysteria2 использует UDP. Управляемый Hiddify/sing-box JSON компилируется из тех же deployment values и детерминированно направляет и DNS, и данные через Hysteria2. Latency-based `urltest` удалён: он выбирает outbound для нового соединения, но не повторяет зависший запрос по другому transport. Static bandwidth и connect timeout не задаются. TLS Hysteria2 проверяется pinned SPKI и именем сертификата; plaintext secret или `insecure` в профиле отсутствуют.
...
- Один статический VLESS URI не может сменить transport и поэтому остаётся совместимым TCP-контрактом. Дополнительный JSON даёт детерминированный QUIC-транспорт для сетей с потерями на TCP-пути; сети без UDP должны использовать основной VLESS URI. Это два явных клиентских контракта к одному RU, а не ложная per-request redundancy. RAW/Reality/Vision не заменяется на XHTTP вслепую: полевые отчёты показывают и обход ISP policing, и отдельные compatibility/latency regressions ([net4people/bbs #546](https://github.com/net4people/bbs/issues/546), [Xray-core #5332](https://github.com/XTLS/Xray-core/issues/5332)).
