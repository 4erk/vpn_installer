...
Дистрибутив и пакетный provider выбираются автоматически после read-only preflight. Указывать ОС отдельным аргументом не нужно.

| Серверная ОС | Пакетный provider |
| --- | --- |
| Ubuntu 22.04/24.04/26.04, Debian 12/13 | `apt` |
| AlmaLinux/Rocky Linux 9/10 | `dnf` (DNF4) |
| Fedora 43/44 | `dnf5` |

Для всех строк матрицы обязательны `x86_64`, `systemd` и отсутствие активного `ufw`/`firewalld`. Установщик не отключает сторонний firewall: он создаёт только собственные таблицы `nftables`. DNS-кеш также принадлежит приложению и запускается отдельным сервисом, не заменяя системный resolver сервера.

Полный контракт платформы, порядок установки пакетов и границы владения описаны в [PLATFORMS.md](./PLATFORMS.md).
...
`--non-interactive` запрещает вопросы и использует connection state. При password-аутентификации пароль берётся из системного хранилища или из `VPN_GATEWAY_SSH_PASSWORD`, `VPN_EXIT_SSH_PASSWORD`, `VPN_SSH_PASSWORD`. Проверка ОС, архитектуры, init system и firewall выполняется так же, как в интерактивном режиме.
